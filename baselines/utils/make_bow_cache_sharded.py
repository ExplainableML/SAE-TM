...
This script uses a sharded, file-based approach for all stages
to support massive corpora.

Workflow
--------
1) Build Vocabulary (Parallel Map-Reduce)

   1.a) Build vocabulary shards in parallel:
        python make_bow_cache.py build-vocab-shard \
          --jsonl data/train.jsonl \
          --out-vocab-shard cache/vocab_shards/shard_0000.pt \
          --start 0 --num-rows 100000

        python make_bow_cache.py build-vocab-shard \
          --jsonl data/train.jsonl \
          --out-vocab-shard cache/vocab_shards/shard_0001.pt \
          --start 100000 --num-rows 100000
        ... (etc.)

   1.b) Merge vocab shards into a final vocab file (serial, fast):
        python make_bow_cache.py merge-vocab-shards \
          --shards "cache/vocab_shards/shard_*.pt" \
          --out-vocab cache/vocab_20000_5.pt \
          --vocab-size 20000 \
          --min-df 5

2) Build BOW Shards (Parallel)
   (This uses the final vocab from step 1.b)

   python make_bow_cache.py build-shard \
     --jsonl data/train.jsonl \
     --vocab cache/vocab_20000_5.pt \
     --out-shard cache/bow_shards/shard_0000.pt \
     --start 0 --num-rows 100000

   python make_bow_cache.py build-shard \
     --jsonl data/train.jsonl \
     --vocab cache/vocab_20000_5.pt \
     --out-shard cache/bow_shards/shard_0001.pt \
     --start 100000 --num-rows 100000
   ... (etc.)

3) Merge BOW Shards (Serial)
   (This uses the final vocab from step 1.b)
...
class DocumentProcessor:
    """
    Optimized Tokenization, filtering, lemmatization (aligned with ETM script).
    Filters tokens *before* running the expensive POS-tagger.
    """
    def __init__(self):
    ...
    def process(self, text: str) -> List[str]:
        ...
        # 2. Run POS-tagging ONCE on the *surviving* tokens
...
class Vocab:
    ...
    # Vocab.build() is removed, as logic is now in merge-vocab-shards
...
def _process_nltk_chunk(doc_chunk: List[str]) -> Tuple[collections.Counter, collections.Counter, int]:
    """Worker function to process a chunk of documents with NLTK."""
    # Each worker initializes its own processor
    # This is required as the processor is not pickle-able
...
def cmd_build_vocab_shard(args):
    """(Parallel) Processes a slice of JSONL and saves its local TF/DF counts."""
    download_nltk_data()
    os.makedirs(os.path.dirname(args.out_vocab_shard) or ".", exist_ok=True)
    
    # processor = DocumentProcessor() # <-- Removed, logic moved to worker
    rows = _slice_jsonl(args.jsonl, args.start, args.num_rows)
    if not rows:
        print(f"[build-vocab-shard] No rows in slice start={args.start} num_rows={args.num_rows}.", file=sys.stderr)
        sys.exit(0)

    # These will aggregate the results from the parallel workers
    global_df = collections.Counter()
    global_tf = collections.Counter()
    global_num_docs = 0
    
    docs = (row.get("document", "") for row in rows)
    
    # --- Multiprocessing setup ---
    num_workers = 8
    # Size of chunks fed to each worker (tune this)
    # A larger chunk size (e.g., 10k-20k) reduces overhead
    chunk_size = 1000  
    doc_chunks = _chunk_iterator(docs, chunk_size)
    
    print(f"[build-vocab-shard] Processing {len(rows)} docs with {num_workers} workers...", file=sys.stderr)

    # NLTK data is downloaded once, workers will inherit it
    with multiprocessing.Pool(num_workers) as pool:
        pbar = tqdm(pool.imap_unordered(_process_nltk_chunk, doc_chunks), 
                    desc=f"VocabShard(start={args.start}, n={len(rows)})", total=len(rows) // chunk_size)
        
        # --- Reduce Step (for this shard) ---
    ...
    print(f"[build-vocab-shard] Parallel processing complete for this shard. Total docs: {global_num_docs}")

    payload = {
        "doc_freq": global_df,
        "term_freq": global_tf,
        "num_docs": global_num_docs,
        "shard_meta": {"start": int(args.start), "count": int(global_num_docs), "jsonl": args.jsonl}
    }
    torch.save(payload, args.out_vocab_shard)
    print(f"[build-vocab-shard] Saved vocab shard to: {args.out_vocab_shard}  (start={args.start}, count={global_num_docs})")


def cmd_merge_vocab_shards(args):
    """(Serial) Merges vocab shards and filters to create the final vocab file."""
    ...
    if not shard_paths:
        print("[merge-vocab-shards] No shard files matched the given patterns.", file=sys.stderr)
        sys.exit(2)
    ...
    print(f"Merging {len(shard_paths)} vocab shards...")
    for p in tqdm(shard_paths, desc="Merging vocab shards"):
        try:
            shard_payload = torch.load(p, map_location="cpu")
            global_df.update(shard_payload["doc_freq"])
            global_tf.update(shard_payload["term_freq"])
            global_num_docs += shard_payload["num_docs"]
        except Exception as e:
            print(f"Warning: Failed to load or merge shard {p}. Error: {e}", file=sys.stderr)

    print(f"Merge complete. Total docs processed: {global_num_docs:,}. Filtering vocab...")

    # --- Filtering logic (from old Vocab.build) ---
    items = [(tok, global_df[tok], global_tf[tok]) for tok in global_df if global_df[tok] >= args.min_df]
    items.sort(key=lambda x: (-x[1], x[0]))
    items = items[:args.vocab_size]
    ...
    vocab = Vocab(token2id, id2token, doc_freq, term_freq, global_num_docs)
    # --- End filtering logic ---

    # Save in the format expected by `cmd_build_shard` and `cmd_merge_shards`
    final_payload = {
        "vocab": vocab.save(), 
        "meta": {"vocab_size": args.vocab_size, "min_df": args.min_df}
    }
    torch.save(final_payload, args.out_vocab)
    print(f"[merge-vocab-shards] Saved final vocab to: {args.out_vocab} (|V|={len(vocab.id2token)})")


def cmd_build_shard(args):
    """(Parallel) Processes a slice of JSONL and saves BOW counts using final vocab."""
    download_nltk_data()
...
def cmd_merge_shards(args):
    """(Serial) Merges final BOW shards into a single dataset for the ETM script."""
...
def build_arg_parser():
    ...
    # 1.a) Build vocab shard
    p_vs = sub.add_parser("build-vocab-shard", help="Create a vocab shard (df/tf) for a slice of the corpus.")
    p_vs.add_argument("--jsonl", required=True, help="Input JSONL with {'id', 'document'} per line.")
    p_vs.add_argument("--out-vocab-shard", required=True, help="Output vocab shard path (.pt).")
    p_vs.add_argument("--start", type=int, required=True, help="0-based starting line index in JSONL.")
    p_vs.add_argument("--num-rows", type=int, required=True, help="Number of rows to process (use -1 for to-the-end).")
    p_vs.set_defaults(func=cmd_build_vocab_shard)
    
    # 1.b) Merge vocab shards
    p_mvs = sub.add_parser("merge-vocab-shards", help="Merge vocab shards into a final vocab file.")
    p_mvs.add_argument(
        "--shards",
        nargs="+",
        required=True,
        help="One or more vocab shard paths or globs (e.g., cache/vocab_shards/shard_*.pt).",
    )
    p_mvs.add_argument("--out-vocab", required=True, help="Output *final* vocab file (.pt).")
    p_mvs.add_argument("--vocab-size", type=int, default=5000)
    p_mvs.add_argument("--min-df", type=int, default=5)
    p_mvs.set_defaults(func=cmd_merge_vocab_shards)


    # 2) Build BOW shard from a slice
    ...
    ps.add_argument("--vocab", required=True, help="*Final* vocab file from merge-vocab-shards (.pt).")
    ps.add_argument("--out-shard", required=True, help="Output BOW shard path (.pt).")
    ...
    # 3) Merge BOW shards
    pm = sub.add_parser("merge-shards", help="Merge multiple BOW shards into a single BowDataset cache.")
    pm.add_argument("--vocab", required=True, help="*Final* vocab file from merge-vocab-shards (.pt).")
...
def main(argv=None):
    multiprocessing.set_start_method("spawn", force=True) # <-- Added
